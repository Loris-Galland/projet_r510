...
// GET /moves
// récupérer tous les moves ou seulement ceux d'un type précis
...
router.get('/', async (req, res) => {
  try {
    ...
    if (nameQuery) {
      query['ename'] = { $regex: `^${nameQuery}`, $options: 'i' };
    }

    const moves = await collection.find(query).toArray(); //exec request + conversion en tab
    res.json(moves);
  } catch (err) {
    console.error(err);
...
// GET /moves/search
// rechercher les moves dont le nom commence par certaines lettres
...
router.get('/search', async (req, res) => {
  try {
    ...
    const query = {
      'ename': { $regex: `^${nameQuery}`, $options: 'i' }
    };
